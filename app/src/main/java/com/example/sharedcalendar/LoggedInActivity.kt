...
class LoggedInActivity : AppCompatActivity() {
    ...
    private fun fetchUserDataFromFirestore(username: String?) {
        ...
        if (username == null) {
            binding.textViewEmail.text = "No username provided!"
            return
        }
        ...
        db.collection("users").document(username).get()
            .addOnSuccessListener { document -> // 성공적으로 데이터를 가져왔을 때의 동작
                if (document.exists()) { // 해당 문서가 존재하면
                    ...
                    binding.textViewEmail.text = "No user data found for $username!"
                }
            }
            .addOnFailureListener { // 데이터를 가져오는데 실패했을 때의 동작
                binding.textViewEmail.text = "Failed to fetch user data. Please try again later."
            }
    }
